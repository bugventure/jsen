...
SchemaResolver.hasRef = function (schema) {
    if (!schema || typeof schema !== 'object') {
        return false;
    }

    // return JSON.stringify(schema).indexOf('"$ref":') > -1;

    var keys, len, i;

    if (Array.isArray(schema)) {
        for (i = 0, len = schema.length; i < len; i++) {
            if (SchemaResolver.hasRef(schema[i])) {
                return true;
            }
        }
    }
        ...
        for (i = 0, len = keys.length; i < len; i++) {
            if (SchemaResolver.hasRef(schema[keys[i]])) {
                return true;
            }
        }
    }
