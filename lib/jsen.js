...
keywords.items = function (context) {
    ...
    if (type(context.schema.items) === 'object') {
        ...
        context.validate(context.path + '[' + index + ']', context.schema.items, context.doNotThrow);
        ...
        for (; i < context.schema.items.length; i++) {
            ...
            context.validate(context.path + '[' + i + ']', context.schema.items[i], context.doNotThrow);
        ...
        if (type(context.schema.additionalItems) === 'object') {
            ...
            context.validate(context.path + '[' + index + ']', context.schema.additionalItems, context.doNotThrow);
...
keywords.properties = function (context) {
    ...
    for (i = 0; i < propKeys.length; i++) {
        ...
        context.validate(context.path + '.' + prop, props[prop], context.doNotThrow);
...
keywords.patternProperties = keywords.additionalProperties = function (context) {
    ...
    for (i = 0; i < propKeys.length; i++) {
        ...
        context.validate(context.path + '.' + propKey, props[propKey], context.doNotThrow);
    ...
    for (i = 0; i < patterns.length; i++) {
        ...
        context.validate(context.path + '[' + key + ']', patProps[pattern], context.doNotThrow);
    ...
    if (addPropsCheck) {
        ...
        if (addProps === false) {
            ...
            // validate additional properties
            context.validate(context.path + '[' + key + ']', addProps, context.doNotThrow);
        }
...
keywords.dependencies = function (context) {
    ...
    for (key in context.schema.dependencies) {
        ...
        if (type(dep) === 'object') {
            //schema dependency
            context.validate(context.path, dep, context.doNotThrow);
        }
        else {
            // property dependency
...
keywords.allOf = function (context) {
    ...
    for (var i = 0; i < context.schema.allOf.length; i++) {
        context.validate(context.path, context.schema.allOf[i], context.doNotThrow);
    }
};
...
keywords.anyOf = function (context) {
    ...
    context.code(initialCount + ' = refs.errors.length');

    for (; i < context.schema.anyOf.length; i++) {
        context.code('if (!' + found + ') {');

        context.code(errCount + ' = refs.errors.length');

        context.validate(context.path, context.schema.anyOf[i], true);

        context.code(found + ' = refs.errors.length === ' + errCount)
        ('}');
    }
    ...
    context.code('} else {')
        ('refs.errors.length = ' + initialCount)
    ('}');
};
...
keywords.oneOf = function (context) {
    ...
    for (; i < context.schema.oneOf.length; i++) {
        ...
        context.code('if (refs.errors.length === ' + errCount + ') {')
            (matching + '++')
        ('}');
    }
...
function jsen(schema) {
    ...
    function compile(schema) {
        ...
        function validate(path, schema, doNotThrow) {
            function error(keyword) {
                ...
                if (path.indexOf('[') > -1) {
                    ...
                    code('refs.errors.push(refs.' + varid + ')');
                }
        ...
        code('return refs.errors.length === 0');

        compiled = code.compile(scope);

        Object.defineProperty(compiled, 'errors', {
            enumerable: true,
            set: function () { },
            get: function () {
                return refs.errors.length ? refs.errors : null;
            }
        });
