...
keywords.multipleOf = function (context) {
    if (typeof context.schema.multipleOf === 'number') {
        ...
        if (decimals > 0) {
            context.code('ok = ok && +(Math.round((' + path + ' * ' + pow + ') + "e+" + ' + decimals + ') + "e-" + ' + decimals + ') % ' + (mul * pow) + ' === 0');
        } else {
            context.code('ok = ok && ((' + path + ' * ' + pow + ') % ' + (mul * pow) + ') === 0');
        }
    }
};
...
keywords.minLength = function (context) {
    if (isInteger(context.schema.minLength)) {
        context.code('ok = ok && ' + context.path + '.length >= ' + context.schema.minLength);
    }
};

keywords.maxLength = function (context) {
    if (isInteger(context.schema.maxLength)) {
        context.code('ok = ok && ' + context.path + '.length <= ' + context.schema.maxLength);
    }
};
...
keywords.pattern = function (context) {
    ...
    if (type(regex) === 'regexp') {
        context.code('ok = ok && (' + regex.toString() + ').test(' + context.path + ')');
    }
};
...
keywords.format = function (context) {
    ...
    context.code('ok = ok && (' + formats[context.schema.format].toString() + ').test(' + context.path + ')');
};

keywords.minItems = function (context) {
    if (isInteger(context.schema.minItems)) {
        context.code('ok = ok && ' + context.path + '.length >= ' + context.schema.minItems);
    }
};

keywords.maxItems = function (context) {
    if (isInteger(context.schema.maxItems)) {
        context.code('ok = ok && ' + context.path + '.length <= ' + context.schema.maxItems);
    }
};

keywords.additionalItems = function (context) {
    if (context.schema.additionalItems === false && Array.isArray(context.schema.items)) {
        context.code('ok = ok && ' + context.path + '.length <= ' + context.schema.items.length);
    }
};

keywords.uniqueItems = function (context) {
    if (context.schema.uniqueItems) {
        context.code('ok = ok && unique(' + context.path + ').length === ' + context.path + '.length');
    }
};
...
keywords.maxProperties = function (context) {
    if (isInteger(context.schema.maxProperties)) {
        context.code('ok = ok && Object.keys(' + context.path + ').length <= ' + context.schema.maxProperties);
    }
};

keywords.minProperties = function (context) {
    if (isInteger(context.schema.minProperties)) {
        context.code('ok = ok && Object.keys(' + context.path + ').length >= ' + context.schema.minProperties);
    }
};
...
['$ref', 'type', 'enum']
    .forEach(function (keyword) { keywords[keyword].start = true; });

['allOf', 'oneOf', 'anyOf', 'not']
    .forEach(function (keyword) { keywords[keyword].end = true; });
        ...
        function validate(path, schema) {
            ...
            for (i = 0; i < gens.length; i++) {
                ...
                if (gen.type && lastType !== gen.type) {
                    ...
                    source('if (ok && ' + types[gen.type](path) + ') {');
                }
