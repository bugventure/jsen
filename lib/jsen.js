...
keywords.items = function (schema, resolver) {
    ...
    if (type.isObject(schema.items)) {
        fu = func('data')
            ('var itemValidator = %s', createValiator(schema.items, resolver).toString())
        ...
        if (type.isObject(schema.additionalItems)) {
            ...
            fu('var ai = %s', createValiator(schema.additionalItems, resolver).toString());
        }
        else {
            fu('var ai = null');
        }
        ...
        schema.items.forEach(function forEachSchema(itemSchema, index) {
            if (type.isObject(itemSchema)) {
                fu('ivs[%d] = %s', index, createValiator(itemSchema, resolver).toString());
            }
        });
...
keywords.properties = function (schema, resolver) {
    ...
    Object.keys(props).forEach(function forEachProp(prop) {
        fu('props["%s"] = %s', prop, createValiator(props[prop], resolver).toString());
    });
...
keywords.patternProperties = function (schema, resolver) {
    ...
    patterns.forEach(function forEachPattern(pattern) {
        var validator = createValiator(patProps[pattern], resolver).toString();
...
keywords.additionalProperties = function (schema, resolver) {
    ...
    var propKeys = type.isObject(schema.properties) ?
        ...
        validator = forbidden ?
            null :
            createValiator(schema.additionalProperties, resolver),
        fu;
...
keywords.dependencies = function (schema, resolver) {
    ...
    Object.keys(schema.dependencies).forEach(function forEach(key) {
        ...
        if (type.isObject(dep)) {
            //schema dependency
            validator = createValiator(dep, resolver).toString();
...
function createValiator(schema, resolver) { // jshint ignore: line
...
jsen = module.exports = function jsen(schema) {
    return createValiator(schema, new SchemaResolver(schema));
};