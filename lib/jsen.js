...
function build(schema, def, options) {
    ...
    if (defType === 'object' && type(schema.properties) === 'object') {
        for (key in schema.properties) {
            if (!def.hasOwnProperty(key)) {
                defValue = build(schema.properties[key], undefined, options);
        ...
        if (type(schema.items) === 'array') {
            for (i = 0; i < schema.items.length; i++) {
                defValue = build(schema.items[i], def[i], options);
            ...
            for (i = 0; i < def.length; i++) {
                def[i] = build(schema.items, def[i], options);
            }
        }
    }
...
function jsen(schema, options) {
    ...
    function compile(schema) {
        ...
        compiled.build = build.bind(null, schema);
