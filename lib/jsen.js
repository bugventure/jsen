...
ValidationContext.prototype.cache = function (schema) {
    var deref = this.resolver.resolve(schema),
        cacheKey = this.resolver.getNormalizedRef(schema) || schema.$ref,
...
ValidationContext.prototype.refactor = function (path, schema) {
    var parentPathExp = path !== 'data' ?
            '(path ? path + "." : "") + ' + getPathExpression(path) :
            'path',
        cachedRef = this.cache(schema),
