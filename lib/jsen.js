...
keywords.minimum = function (schema) {
    if (type.isNumber(schema.minimum)) {
        return 'ok = !(' + types.number() + ') || data >= ' + schema.minimum;
    }
...
keywords.maximum = function (schema) {
    if (type.isNumber(schema.maximum)) {
        return 'ok = !(' + types.number() + ') || data <= ' + schema.maximum;
    }
...
keywords.multipleOf = function (schema) {
    if (type.isNumber(schema.multipleOf)) {
        ...
        return 'ok = !(' + types.number() + ') || ((data * ' + pow + ') % ' + (mul * pow) + ') === 0';
    }
...
keywords.minLength = function (schema) {
    if (type.isInteger(schema.minLength)) {
        return 'ok = !(' + types.string() + ') || data.length >= ' + schema.minLength;
    }
...
keywords.maxLength = function (schema) {
    if (type.isInteger(schema.maxLength)) {
        return 'ok = !(' + types.string() + ') || data.length <= ' + schema.maxLength;
    }
...
keywords.pattern = function (schema) {
    ...
    if (type.isRegExp(regex)) {
        return 'ok = !(' + types.string() + ') || (' + regex.toString() + ').test(data)';
    }
...
keywords.format = function (schema) {
    ...
    return 'ok = !(' + types.string() + ') || (' + formats[schema.format].toString() + ').test(data)';
};

keywords.minItems = function (schema) {
    if (type.isInteger(schema.minItems)) {
        return 'ok = !(' + types.array() + ') || data.length >= ' + schema.minItems;
    }
...
keywords.maxItems = function (schema) {
    if (type.isInteger(schema.maxItems)) {
        return 'ok = !(' + types.array() + ') || data.length <= ' + schema.maxItems;
    }
...
keywords.uniqueItems = function (schema) {
    if (schema.uniqueItems) {
        return 'ok = !(' + types.array() + ') || unique(data).length === data.length';
    }

    return;
};

keywords.items = function (schema, compile, id) {
    var fu = func(),
        i = id(),
        validator,
        schemas,
    ...
    if (type.isObject(schema.items)) {
        ...
        if (type.isObject(schema.additionalItems)) {
            ...
            fu.def(aiv, compile(schema.additionalItems));
        }
        else {
            fu.def(aiv, 'null');
        }
        ...
        fu.def(schemas, '[%s]', schema.items.map(function mapSchema(itemSchema) {
...
keywords.additionalItems = function (schema) {
    if (schema.additionalItems === false && type.isArray(schema.items)) {
        return 'ok = !(' + types.array() + ') || data.length <= ' + schema.items.length;
    }
...
keywords.maxProperties = function (schema) {
    if (type.isInteger(schema.maxProperties)) {
        return 'ok = !(' + types.object() + ') || keys.length <= ' + schema.maxProperties;
    }
...
keywords.minProperties = function (schema) {
    if (type.isInteger(schema.minProperties)) {
        return 'ok = !(' + types.object() + ') || keys.length >= ' + schema.minProperties;
    }

    return;
};

keywords.required = function (schema, compile, id) {
    ...
    fu.def(i, 0)
      .def(required, JSON.stringify(schema.required));

    fu('if (%s) {', types.object())
...
keywords.properties = function (schema, compile, id) {
    ...
    fu.def(propvalidators, '{ ' + Object.keys(props).map(function mapProp(prop) {
...
keywords.patternProperties = function (schema, compile, id) {
    ...
    fu.def(patternValidators, '{ ' + patterns.map(function mapPattern(pattern) {
...
keywords.additionalProperties = function (schema, compile, id) {
    ...
    fu = func()
        .def(properties, JSON.stringify(propKeys))
        .def(key, '""')
        .def(found, 'false')
        .def(regexes, JSON.stringify(patternRegexes))
        .def(i, 0)
        .def(j, 0)
        ('if (%s) {', types.object());
...
keywords.allOf = function (schema, compile, id) {
    ...
    fu.def(validators, '[' + schema.allOf.map(function mapSchema(childSchema) {
        return compile(childSchema);
    }).join(', ') + ']');

    fu('for (%s; %s < %s.length; %s++) {', i, i, validators, i)
...
keywords.anyOf = function (schema, compile, id) {
    ...
    fu.def(i, 0).def(found, 'false');

    fu.def(validators, '[' + schema.anyOf.map(function mapSchema(childSchema) {
        return compile(childSchema);
    }).join(', ') + ']');
...
keywords.oneOf = function (schema, compile, id) {
    ...
    fu.def(i, 0).def(matching, 0);

    fu.def(validators, '[' + schema.oneOf.map(function mapSchema(childSchema) {
        return compile(childSchema);
    }).join(', ') + ']');

    fu('for (%s = 0; %s < %s.length; %s++) {', i, i, validators, i)
        ('if (%s[%s](data)) {', validators, i)
            ('%s++', matching)
            ('if (%s > 1) {', matching)
                ('ok = false')
...
function createValidator(schema) {
    ...
    function compile(schema) {
        ...
        for (i = 0; i < keys.length; i++) {
            kw = keys[i];
            src = (keywords[kw] || noop)(schema, cacheCompile, id);
