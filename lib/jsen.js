...
keywords.patternProperties = keywords.additionalProperties = function (context) {
    ...
    for (i = 0; i < propKeys.length; i++) {
        ...
        context.descend(appendToPath(context.path, propKey), props[propKey]);
...
keywords.anyOf = function (context) {
    ...
    for (; i < context.schema.anyOf.length; i++) {
        ...
        context.descend(context.path, context.schema.anyOf[i], true);
...
keywords.oneOf = function (context) {
    ...
    for (; i < context.schema.oneOf.length; i++) {
        ...
        context.descend(context.path, context.schema.oneOf[i], true);
...
keywords.not = function (context) {
    ...
    context.descend(context.path, context.schema.not, true);
...
function ValidationContext(options) {
    ...
    this.noFailFast = options.noFailFast || false;
...
ValidationContext.prototype.clone = function (schema, noFailFast) {
...
ValidationContext.prototype.id = function () {
...
ValidationContext.prototype.getCacheKey = function (schema) {
    return JSON.stringify(schema);
};

ValidationContext.prototype.cache = function (schema, noFailFast) {
    var deref = this.resolver.resolve(schema),
        cacheKey = this.getCacheKey(deref),
    ...
    if (!cached) {
        ...
        context = this.clone(deref, noFailFast);

        cached.func = context.compile();
...
ValidationContext.prototype.error = function (keyword, key, additional) {
    var schema = this.schema,
        path = this.path,
        errorPath = getPathExpression(path, key),
    ...
    if (!this.noFailFast && !this.greedy) {
        this.code('return errors');
    }
};

ValidationContext.prototype.descend = function (path, schema, noFailFast) {
    var noFail = noFailFast !== undefined ? noFailFast : this.noFailFast,
        parentPathExp = getPathExpression(path),
        cachedRef = this.cache(schema, noFail),
        refErrors = this.declare();

    this.code(refErrors + ' = ' + cachedRef + '(' + path + ', ' + parentPathExp + ')')
    ('if (' + refErrors + '.length > 0) {')
        ('errors = errors.concat(' + refErrors + ')');
        // ('console.log(errors)');

    if (!noFail && !this.greedy) {
        this.code('return errors');
    }
...
ValidationContext.prototype.compile = function () {
    this.code = func('validate_' + this.id(), 'data');
    this.code.def('errors', '[]');
    this.generate();
    this.code('return errors');
...
function jsen(schema, options) {
    ...
    var resolver = new SchemaResolver(schema, options.schemas, options.missing$Ref || false),
