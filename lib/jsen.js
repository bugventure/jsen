...
function createValidator(schema, resolver, counter, cache) { // jshint ignore: line
    if (!type.isObject(schema)) {
        throw new Error(strings.invalidSchema);
    }

    var fu = func('data'),
        counter = counter || 0, // jshint ignore: line
        cache = cache || {},    // jshint ignore: line
        keys, kw, src, i,
        compiled;

    function compile(schema) {
        var deref = resolver.resolve(schema),
            compiled = createValidator(deref, resolver, counter, cache);
    ...
    for (i = 0; i < keys.length; i++) {
        kw = keys[i];
        src = (keywords[kw] || noop)(schema, compile);
