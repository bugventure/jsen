...
keywords.items = function (schema, resolver) {
    var fu;

    if (type.isObject(schema.items)) {
        fu = func('data')
            ('var itemValidator = %s', createValidator(schema.items, resolver).toString())
        ...
        if (type.isObject(schema.additionalItems)) {
            ...
            fu('var ai = %s', createValidator(schema.additionalItems, resolver).toString());
        }
        else {
            fu('var ai = null');
        }
        ...
        schema.items.forEach(function forEachSchema(itemSchema, index) {
            if (type.isObject(itemSchema)) {
                fu('ivs[%d] = %s', index, createValidator(itemSchema, resolver).toString());
            }
        });
...
keywords.properties = function (schema, resolver) {
    ...
    Object.keys(props).forEach(function forEachProp(prop) {
        fu('props["%s"] = %s', prop, createValidator(props[prop], resolver).toString());
    });
...
keywords.patternProperties = function (schema, resolver) {
    ...
    patterns.forEach(function forEachPattern(pattern) {
        var validator = createValidator(patProps[pattern], resolver).toString();
...
keywords.additionalProperties = function (schema, resolver) {
    ...
    var propKeys = type.isObject(schema.properties) ?
        ...
        validator = forbidden ?
            null :
            createValidator(schema.additionalProperties, resolver),
        fu;
    ...
    if (validator) {
        fu('var validator = %s', validator.toString());
    }
...
keywords.dependencies = function (schema, resolver) {
    ...
    Object.keys(schema.dependencies).forEach(function forEach(key) {
        ...
        if (type.isObject(dep)) {
            //schema dependency
            validator = createValidator(dep, resolver).toString();
...
keywords.allOf = function (schema, resolver) {
    ...
    schema.allOf.forEach(function forEachSchema(childSchema) {
        fu('validators.push(%s)', createValidator(childSchema, resolver).toString());
    });
...
keywords.anyOf = function (schema, resolver) {
...
keywords.oneOf = function (schema, resolver) {
...
keywords.not = function (schema, resolver) {
    if (!type.isObject(schema.not)) {
        return 'true';
    }

    return '!(' + createValidator(schema.not, resolver).toString() + ')(data)';
};
...
function walk(schema, fu, resolver) {
    ...
    for (i = 0; i < keys.length; i++) {
        kw = keys[i];
        src = (keywords[kw] || noop)(schema, resolver);
...
function createValidator(schema, resolver) { // jshint ignore: line
...
jsen = module.exports = function jsen(schema) {
    return createValidator(schema, new SchemaResolver(schema));
};