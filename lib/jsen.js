...
keywords.items = function (context) {
    ...
    context.code('if (ok && %s) {', types.array(context.path));

    if (type(context.schema.items) === 'object') {
        context.code('for (%s; %s < ' + context.path + '.length; %s++) {', index, index, index);
        ...
        for (; i < context.schema.items.length; i++) {
            context.code('if (%s.length - 1 >= %s) {', context.path, i);
        ...
        if (type.isObject(context.schema.additionalItems)) {
            context.code('for (%s = %s; %s < ' + context.path + '.length; %s++) {', index, i, index, index);
...
keywords.additionalItems = function (context) {
    if (context.schema.additionalItems === false && Array.isArray(context.schema.items)) {
        context.code('ok = ok && !(%s) || %s.length <= %s', types.array(context.path), context.path, context.schema.items.length);
    }
};

keywords.maxProperties = function (context) {
    if (type.isInteger(context.schema.maxProperties)) {
        context.code('ok = ok && !(%s) || Object.keys(%s).length <= %s', types.object(context.path), context.path, context.schema.maxProperties);
    }
};

keywords.minProperties = function (context) {
    if (type.isInteger(context.schema.minProperties)) {
        context.code('ok = ok && !(%s) || Object.keys(%s).length >= %s', types.object(context.path), context.path, context.schema.minProperties);
    }
};
...
keywords.required = function (context) {
    ...
    context.code('if (ok && %s) {', types.object(context.path));

    for (var i = 0; i < context.schema.required.length; i++) {
        context.code('ok = ok && %s.%s !== undefined', context.path, context.schema.required[i]);
    }
...
keywords.properties = function (context) {
    ...
    for (prop in props) {
        context.code('if (%s.%s !== undefined) {', context.path, prop);
...
keywords.$ref = function (context) {
    ...
    context.code('ok = ok && %s(%s)', compiled, context.path);
};

// TODO: Object validators / multi validators
