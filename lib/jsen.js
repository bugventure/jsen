...
function build(schema, def, additional) {
    ...
    if (defType === 'object' && type(schema.properties) === 'object') {
        for (key in schema.properties) {
            defValue = build(schema.properties[key], def[key], additional);
        ...
        for (key in def) {
            if (!(key in schema.properties) && (additional === false || schema.additionalProperties === false)) {
                delete def[key];
            }
        }
    }
        ...
        if (type(schema.items) === 'array') {
            for (i = 0; i < schema.items.length; i++) {
                defValue = build(schema.items[i], def[i], additional);
            ...
            for (i = 0; i < def.length; i++) {
                def[i] = build(schema.items, def[i], additional);
            }
        }
    }
