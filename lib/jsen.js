...
function build(schema, def, additional, resolve, parentMarker) {
    ...
    if (defType === 'object' && type(schema.properties) === 'object') {
        ...
        for (i = 0; i < props.length; i++) {
            key = props[i];
            defValue = build(schema.properties[key], def[key], additional, resolve);
        ...
        if (type(schema.items) === 'array') {
            for (i = 0; i < schema.items.length; i++) {
                defValue = build(schema.items[i], def[i], additional, resolve);
            ...
            for (i = 0; i < def.length; i++) {
                def[i] = build(schema.items, def[i], additional, resolve);
            }
        }
    }
        ...
        for (i = 0; i < schema.allOf.length; i++) {
            def = build(schema.allOf[i], def, additional, resolve, propertyMarker);
        }
...
ValidationContext.prototype.error = function (keyword, key, additional) {
    var schema = this.schema,
        ...
        res = schema.properties ?
            (this.resolver ?
                this.resolver.resolve(schema.properties[key]) :
...
ValidationContext.prototype.generate = function () {
    ...
    if (schema.$ref !== undefined) {
        ...
        if (SchemaResolver.hasRef(schema)) {
...
function jsen(schema, options) {
    ...
    var counter = 0,
        ...
        resolver = SchemaResolver.hasRef(schema) ?
            new SchemaResolver(schema, options.schemas, options.missing$Ref || false) :
            undefined,
