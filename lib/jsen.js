...
keywords.missing$Ref = function (context) {
    context.missing(context.schema.missing$Ref);
};
...
function jsen(schema, options) {
    ...
    function cache(schema) {
        ...
        if (!cached) {
            ...
            Object.defineProperty(cached.func, 'missing', {
                get: function () {
                    return func.missing;
                }
            });
    ...
    function compile(schema) {
        ...
        function validate(path, schema, noFailFast) {
            ...
            function error(keyword, key) {
                ...
                if (missing$Ref) {
                    code('validate.missing = Object.keys(missing)');
                }
            ...
            if (schema.$ref !== undefined) {
                ...
                if (missing$Ref) {
                    code('if (' + cachedRef + '.missing) {')
                        ('for (' + j + ' = 0; ' + j + ' < ' + cachedRef + '.missing.length; ' + j + '++) {')
                            ('missing[' + cachedRef + '.missing[' + j + ']] = true')
                        ('}')
                    ('}');
                }
